...
const BOARD_ZOBRIST_SEED: u64 = 0x5408_12DB_8157_5EED;
const SIDE_TO_MOVE_ZOBRIST_SEED: u64 = 0x3C6E_0B8A_9D2F_1A7B;
const HAND_ZOBRIST_SEED: u64 = 0xA1B2_C3D4_E5F6_7890;

/// Zobrist hash table for board pieces: [piece_type][color][square]
static BOARD_HASH: LazyLock<[[[ZobristHash; NUM_SQUARES]; NUM_COLORS]; NUM_PIECE_TYPES]> =
    LazyLock::new(|| {
        let mut board_hash = [[[0u64; NUM_SQUARES]; NUM_COLORS]; NUM_PIECE_TYPES];
        let mut rng = ChaCha8Rng::seed_from_u64(BOARD_ZOBRIST_SEED);
...
static SIDE_TO_MOVE_HASH: LazyLock<ZobristHash> = LazyLock::new(|| {
    let mut rng = ChaCha8Rng::seed_from_u64(SIDE_TO_MOVE_ZOBRIST_SEED);
    rng.gen()
});

/// Zobrist hash table for hand pieces: [piece_type_index][color][count]
static HAND_HASH: LazyLock<[[[u64; MAX_HAND_COUNT]; NUM_COLORS]; NUM_HAND_PIECE_TYPES]> =
    LazyLock::new(|| {
        let mut hand_hash = [[[0u64; MAX_HAND_COUNT]; NUM_COLORS]; NUM_HAND_PIECE_TYPES];
        let mut rng = ChaCha8Rng::seed_from_u64(HAND_ZOBRIST_SEED);
...
pub fn board_hash(piece: Piece, sq: Square) -> u64 {
    BOARD_HASH[piece.piece_type.index()][piece.color.index()][sq.index()]
}
...
pub fn hand_hash(piece: Piece, count: u8) -> u64 {
    if let Some(idx) = hand_piece_index(piece.piece_type) {
        HAND_HASH[idx][piece.color.index()][count as usize]
    } else {
        0
    }
}
